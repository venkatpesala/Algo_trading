...
class KiteApp:
    ...
    def instruments_Banknifty(self, exchange=None):
        data = self.session.get(f"{self.root_url}/instruments",headers=self.headers).text
        pattern = re.compile(r'.,BANKNIFTY23N0843600[C|P]E.')
    ...
    def place_order(self, variety, exchange, tradingsymbol, transaction_type, quantity, product, order_type, price=100,
    ...
    def modify_spotprice(self,input_string, delta):
        # Use regular expression to find the last five digits in the string
        modified_string = re.sub(r'\d{5}(?=[^\d]*$)', lambda x: str(int(x.group()) + delta).zfill(5), input_string)
        
        return modified_string
    


    def spot_symbols_bn(self,spot_price):
        ...
        # from datetime import datetime
        # # Convert the text to list of tuples (string, date)
        # data = [line.split(', ') for line in matched_strings]
        # data = [(item[0], datetime.strptime(item[1], '%Y-%m-%d')) for item in data]

        # # Sort the data by date in ascending order
        # sorted_data = sorted(data, key=lambda x: x[1])
        # Sort the list based on the extracted date using a lambda function
        sorted_data = sorted(matched_strings, key=lambda s: s.split(',')[1])

        # Convert the sorted data back to the original format
        # sorted_text = [f'{item[0]}, {item[1].strftime("%Y-%m-%d")}' for item in sorted_data]
